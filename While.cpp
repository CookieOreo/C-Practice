/*
Author: Raymond Chen 
Description: While (user == gullible). Mon June 8th 2015
...
int main(){
	...
	int number = 0;
	do{
		cout << "Enter any number other than " << number << "."<< endl;
		cin >> trap;
		if (trap != number) count ++;
		if (count == 10) break;
		number ++;
	} while (trap != number - 1);
	if (trap == number - 1){
		cout << "Hey! you weren't supposed to enter " << number - 1 << "!" << endl;
	}
	else if (count == 10){
